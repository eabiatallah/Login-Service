...
//		emailSender.send(request.getEmail(), buildEmail(request.getFirstName(), link));
		...
		return new RegistrationResponse(token, Boolean.TRUE);
	...
	public RegistrationResponse confirmToken(String token) {
		...
		if (confirmationToken.getConfirmedAt() != null) {
			return new RegistrationResponse("email already confirmed", Boolean.FALSE);
		}

		LocalDateTime expiredAt = confirmationToken.getExpiresAt();

		if (expiredAt.isBefore(LocalDateTime.now())) {
			return new RegistrationResponse("Token is expired", Boolean.FALSE);
		}

		confirmationTokenService.setConfirmedAt(token);
		appUserService.enableAppUser(confirmationToken.getAppUser().getEmail());
		return new RegistrationResponse("Email enabled", Boolean.TRUE);
	}
